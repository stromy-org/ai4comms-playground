...
Every presentation MUST use brand data from `client-data`. This skill does not support unbranded decks — use the standard `pptx` skill for those.

### Brand artifact inventory

When the user names a client, load all artifacts from `client-data/clients/<slug>/`:
