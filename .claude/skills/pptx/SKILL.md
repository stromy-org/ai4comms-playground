...
cp .claude/skills/pptx/scripts/build-branded.js workspace/my-deck/build.js
```
...
When a company directory exists, also check for `profile.json` at `client-data/clients/<name>/profile.json`. If present, use company identity fields for presentation slides:
...
When a company directory exists, check for `people.json` at `client-data/clients/<name>/people.json`. If present, use it for author metadata in footers, contact slides, and file properties. Filter by `roles` containing `"author"` — if one person has `"default": true`, auto-select them.
